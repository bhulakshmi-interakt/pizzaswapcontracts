...
contract Lottery is Ownable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        AGF.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        require(_price >= minPrice, "price must above minPrice");
        require(
            _numbers.length <= maxNumber,
            "buy ticket number count > maxNumber"
        );
        ...
        AGF.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            safeAGFTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            safeAGFTransfer(address(msg.sender), totalReward);
        }
    ...
    ) public pure returns (uint32[winningCombinations] memory) {
    ...
    // Safe AGF transfer function, just in case if rounding error causes pool to not have enough AGFs.
    function safeAGFTransfer(address _to, uint256 _amount) internal {
        uint256 AGFBal = AGF.balanceOf(address(this));
        if (_amount > AGFBal) {
            AGF.transfer(_to, AGFBal);
        } else {
            AGF.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        AGF.safeTransfer(address(msg.sender), _amount);
